...
parser.add_argument( "--model_type", type = str, default = "SaN", help = "Choosing the model to be used for training. Options: MN2, IC3, ENB1." )
...
if __name__ == '__main__':
  ...
  for fold_index in range( 0, NUMBER_OF_FOLDS ):
    ...
    base_model = None
    if CUSTOM_MODEL_TYPE == 'SaN':
      base_model = model_creator.sketchANetWithNewInput( pretrained_weights_path, IMAGE_HEIGHT, IMAGE_WIDTH )
    elif CUSTOM_MODEL_TYPE == 'MN2':
      base_model = model_creator.mobileNetV2WithNewInput( pretrained_weights_path, IMAGE_HEIGHT, IMAGE_WIDTH )
    elif CUSTOM_MODEL_TYPE == 'IC3':
      base_model = model_creator.inceptionV3WithNewInput( pretrained_weights_path, IMAGE_HEIGHT, IMAGE_WIDTH )
    elif CUSTOM_MODEL_TYPE == 'ENB1':
      base_model = model_creator.efficientNetB1WithNewInput( pretrained_weights_path, IMAGE_HEIGHT, IMAGE_WIDTH )
    ...
    # Adding a new top to the models. 
      
    model = None
    
    if CUSTOM_MODEL_TYPE == 'SaN': 
    
      model = model_creator.sketchANetAddTop( base_model, IMAGE_HEIGHT, IMAGE_WIDTH )
      
    elif CUSTOM_MODEL_TYPE == 'MN2' or CUSTOM_MODEL_TYPE == 'IC3':
